class ImageConstant {
  // Image folder path
  static String imagePath = 'assets/images';

  // Launch screen images
  static String imgScreenshot20231212 =
      '$imagePath/img_screenshot_2023_12_12.png';

  // Sign up images
  static String imgMdilightemail = '$imagePath/img_mdilightemail.svg';

  static String imgGoogle = '$imagePath/img_google.svg';

  static String imgApple = '$imagePath/img_apple.svg';

  static String imgLogoFacebook = '$imagePath/img_logo_facebook.svg';

  // Search - Where to go images
  static String imgImage13 = '$imagePath/img_image_13.png';

  static String imgImage11 = '$imagePath/img_image_11.png';

  static String imgImage12 = '$imagePath/img_image_12.png';

  // Search - When to go images
  static String imgIDelete = '$imagePath/img_i_delete.svg';

  static String imgIAdd = '$imagePath/img_i_add.svg';

  // Search - Who to go images
  static String imgEDelete = '$imagePath/img_e_delete.svg';

  static String imgIAddBlueGray800 = '$imagePath/img_i_add_blue_gray_800.svg';

  // Search results images
  static String imgSetupPreferences = '$imagePath/img_setup_preferences.svg';

  static String imgHeartGray500 = '$imagePath/img_heart_gray_500.svg';

  // House details - Overview images
  static String imgImage34 = '$imagePath/img_image_34.png';

  static String imgArrowLeftOnprimarycontainer =
      '$imagePath/img_arrow_left_onprimarycontainer.svg';

  static String imgHeartOnprimarycontainer =
      '$imagePath/img_heart_onprimarycontainer.svg';

  static String imgImage = '$imagePath/img_image.svg';

  static String imgLoveHeartPin1 = '$imagePath/img_love_heart_pin_1.svg';

  static String imgRectangle36x36 = '$imagePath/img_rectangle_36x36.png';

  static String img111 = '$imagePath/img_11_1.png';

  static String imgRectangle1 = '$imagePath/img_rectangle_1.png';

  static String imgRating11 = '$imagePath/img_rating_11.png';

  static String imgImage35 = '$imagePath/img_image_35.png';

  // House details - All reviews images
  static String imgRectangle2 = '$imagePath/img_rectangle_2.png';

  static String imgRectangle3 = '$imagePath/img_rectangle_3.png';

  static String imgRectangle4 = '$imagePath/img_rectangle_4.png';

  static String imgRectangle5 = '$imagePath/img_rectangle_5.png';

  // House details - Description images
  static String imgImage42 = '$imagePath/img_image_42.png';

  static String imgImage8335x162 = '$imagePath/img_image_8_335x162.png';

  static String imgImage43 = '$imagePath/img_image_43.png';

  // Checkout images
  static String imgImage45 = '$imagePath/img_image_45.png';

  static String imgEdit1 = '$imagePath/img_edit_1.svg';

  // Payment success images
  static String imgImage46 = '$imagePath/img_image_46.png';

  // Wishlist images
  static String imgMenu510 = '$imagePath/img_menu_5_10.svg';

  // User profile images
  static String imgRectangle120x120 = '$imagePath/img_rectangle_120x120.png';

  static String imgCreditCard = '$imagePath/img_credit_card.svg';

  static String imgSettings = '$imagePath/img_settings.svg';

  static String imgHeadset = '$imagePath/img_headset.svg';

  // Edit profile images
  static String imgImage46142x142 = '$imagePath/img_image_46_142x142.png';

  static String imgEdit = '$imagePath/img_edit.svg';

  // Add a Place images
  static String imgVector = '$imagePath/img_vector.svg';

  static String imgVectorPrimarycontainer =
      '$imagePath/img_vector_primarycontainer.svg';

  static String imgVectorPrimarycontainer2x17 =
      '$imagePath/img_vector_primarycontainer_2x17.svg';

  // Common images
  static String imgLogo = '$imagePath/img_logo.png';

  static String imgProfile = '$imagePath/img_profile.svg';

  static String imgChatCircleDots = '$imagePath/img_chat_circle_dots.svg';

  static String imgBell1 = '$imagePath/img_bell_1.svg';

  static String imgRectangle = '$imagePath/img_rectangle.png';

  static String imgSearch = '$imagePath/img_search.svg';

  static String imgImage8 = '$imagePath/img_image_8.png';

  static String imgStar = '$imagePath/img_star.svg';

  static String imgImage7 = '$imagePath/img_image_7.png';

  static String imgImage9 = '$imagePath/img_image_9.png';

  static String imgImage14 = '$imagePath/img_image_14.png';

  static String imgNavHome = '$imagePath/img_nav_home.svg';

  static String imgNavMyLists = '$imagePath/img_nav_my_lists.svg';

  static String imgNavBooking = '$imagePath/img_nav_booking.svg';

  static String imgNavInbox = '$imagePath/img_nav_inbox.svg';

  static String imgNavProfile = '$imagePath/img_nav_profile.svg';

  static String imgERemove = '$imagePath/img_e_remove.svg';

  static String imgSearchOnprimary = '$imagePath/img_search_onprimary.svg';

  static String imgArrowLeft = '$imagePath/img_arrow_left.svg';

  static String imgImage22 = '$imagePath/img_image_22.png';

  static String imgHeart = '$imagePath/img_heart.svg';

  static String imgMenu53 = '$imagePath/img_menu_5_3.svg';

  static String imgRating3 = '$imagePath/img_rating_3.png';

  static String imgImage23 = '$imagePath/img_image_23.png';

  static String imgWifi1 = '$imagePath/img_wifi_1.svg';

  static String imgPan1 = '$imagePath/img_pan_1.svg';

  static String imgSwimmingPool1 = '$imagePath/img_swimming_pool_1.svg';

  static String imgPlantGround1 = '$imagePath/img_plant_ground_1.svg';

  static String imgArrowright = '$imagePath/img_arrowright.svg';

  static String imgClock = '$imagePath/img_clock.svg';

  static String imgBathtub1 = '$imagePath/img_bathtub_1.svg';
  static String imgPassword = '$imagePath/img_password.png';

  static String imageNotFound = 'assets/images/image_not_found.png';
}
